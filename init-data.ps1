...
function HashPassword($password) {
    ...
    $pbkdf2.Dispose()
    return @{ Hash = [Convert]::ToBase64String($hash); Salt = [Convert]::ToBase64String($salt) }
}
...
if ($batch.Count -gt 0) { $students.InsertBulk($batch) | Out-Null }
...
$total = 0.0; $count = 0
foreach ($s in $students.FindAll()) { $total += $s["Score"].AsDouble; $count++ }
